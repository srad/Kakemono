...
// Parabolic arc 6:00 → 18:00. Returns {x, y, visible} with x,y in [0,1].
...
// Moon arc 18:00 → 06:00 (next day). Returns {x, y, visible}.
export function moonPosition(hour) {
  let t;
  if (hour >= 18) t = (hour - 18) / 12;
  else if (hour < 6) t = (hour + 6) / 12;
  else return { x: 0, y: 0, visible: 0 };
  return { x: t, y: Math.sin(Math.PI * t), visible: 1 };
}

// dawn:[5,9), day:[9,17), dusk:[17,20), night:[20,24)∪[0,5).
...
const ClockTick = {
  ...
  render() {
    ...
    if (widget) {
      ...
      const sun = sunPosition(hourFrac);
      const moon = moonPosition(hourFrac);
